...
def calcAccuracy(predict, label):####dengbin
    """
    ...
    # print("%.3f" % Kappa)
    return OA, Kappa, producerA,array
...
def sample_gt(gt, train_size, mode='random'):
    ...
    """
    tr_indices = np.nonzero(gt['Tr'])
    tr_X = list(zip(*tr_indices))
    te_indices = np.nonzero(gt['Te'])
    te_X = list(zip(*te_indices))
    # indices = np.nonzero(gt)
    # X = list(zip(*indices)) # x,y features
    # y = gt[indices].ravel() # classes
    train_gt = np.zeros_like(gt['Tr'])
    test_gt = np.zeros_like(gt['Te'])
    if train_size > 1:
    ...
    if mode == 'random':
       ...
       test_gt[test_indices] = gt[test_indices]
        ...
        train_indices = tr_X
        train_indices = [list(t) for t in zip(*train_indices)]
        train_gt[train_indices] = gt['Tr'][train_indices]

        test_indices = te_X
        test_indices = [list(t) for t in zip(*test_indices)]
        test_gt[test_indices] = gt['Te'][test_indices]
    else:
        raise ValueError("{} sampling is not implemented yet.".format(mode))
    return train_gt, test_gt

def split_dataset_equal(gt_2D, train_size1, train_size2):
    ...
    for i in range(1, numClass + 1):  # 忽略背景元素
        ...
        if i ==1or i==2:
            train_idx.append(idx[:(math.floor(len(idx)*train_size1))])  # 样本不足，收集每一类的前n/2个作为训练样本
            test_idx.append(idx[(math.floor(len(idx)*train_size1)):])  # 收集每一类剩余的作为测试样本
        elif i ==3or i==4:
            train_idx.append(idx[:(math.floor(len(idx) * train_size2))])  # 样本不足，收集每一类的前n/2个作为训练样本
            test_idx.append(idx[(math.floor(len(idx) * train_size2)):])  # 收集每一类剩余的作为测试样本
    ...
    # io.savemat('./Data/IndianPines/IndianPines_eachClass_30_train.mat', {'data': train_gt_2D})  #保存mat文件
    # io.savemat('./Data/IndianPines/IndianPines_eachClass_30_test.mat', {'data': test_gt_2D})
    return train_gt_2D, test_gt_2D

def sample_gt1(gt, train_size, mode='random'):  # [500,350]:1,2,3,4
    """Extract a fixed percentage of samples from an array of labels.
    ...
    indices = np.nonzero(gt)  # 返回非零元素的索引。
    X = list(zip(*indices))  # x,y features
    y = gt[indices].ravel() # classes
    train_gt = np.zeros_like(gt)  # [500,350] # 返回与给定数组具有相同形状和类型的零数组。
    test_gt = np.zeros_like(gt)   # [500,350]
    if train_size > 1:
       train_size = int(train_size)

    if mode == 'random':  # 按照比例（train——size），选择训练，测试样本
       train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y)
       train_indices = [list(t) for t in zip(*train_indices)]
       test_indices = [list(t) for t in zip(*test_indices)]
       ...
       test_gt[test_indices] = gt[test_indices]
        ...
        train_gt, test_gt = split_dataset_equal(gt,train_size1=train_size*5,train_size2=train_size)
